# aiomtec2mqtt - Asynchronous M-TEC to MQTT Bridge

## About this Fork

This project is a fork of the fantastic [croedel/MTECmqtt](https://github.com/croedel/MTECmqtt) project. We want to express our sincere gratitude to Christian Rödel for his excellent foundational work and the effort that made this integration possible in the first place.

### Why a Fork?

While the original `mtec2mqtt` project works reliably, we created this fork to modernize the codebase and add enhanced capabilities:

**🚀 Asynchronous Architecture**

- Complete rewrite using Python's `asyncio` for non-blocking I/O operations
- Improved responsiveness and resource efficiency, especially on low-power devices (Raspberry Pi, NAS)
- Better handling of concurrent Modbus requests and MQTT publishing

**🔧 Modern Python Standards**

- Full Python 3.13+ support with strict type checking (mypy)
- Async/await patterns throughout the codebase for better maintainability
...
- Asynchronous Modbus register scanning reduces blocking operations
- Parallel MQTT publishing for multiple topics
- Reduced latency for real-time data monitoring

**🔄 Continued Evolution**

- Active maintenance with modern development practices
- Regular updates to support latest Home Assistant and evcc versions
- Community-driven feature development

We maintain full compatibility with the original project's MQTT topic structure and register definitions to ensure seamless migration for existing users.

## Introduction

Welcome to the `aiomtec2mqtt` project!

This project enables you to read data from a M-TEC Energybutler (https://www.mtec-systems.com) hybrid inverter system and publish it to an MQTT broker for integration with home automation and energy management systems.

**Key Features:**

- No additional hardware or modifications to your inverter required
- Lightweight installation on any server (Raspberry Pi, NAS, VM)
- Works entirely within your local network - no internet connection required
- Monitors 80+ inverter parameters in real-time
- Intelligent register clustering to minimize Modbus traffic
- Asynchronous operation for high-frequency polling (every 10s) without blocking
- MQTT integration with almost any EMS or home automation platform
- Home Assistant auto-discovery support
- Pre-built dashboard templates included
- Ready-to-use evcc integration for PV surplus charging
...
```
mkdir aiomtec2mqtt && cd aiomtec2mqtt
```
...
(4) Install aiomtec2mqtt from PyPI

```
pip install aiomtec2mqtt
```

This is all you need to do for a standard setup!

**Optional:** To install the latest development version from GitHub instead:
...
For a Linux system it's probably `~/.config/aiomtec2mqtt/config.yaml`, on Windows something like `C:\Users\xxxxx\AppData\Roaming\aiomtec2mqtt\config.yaml`
...
`aiomtec2mqtt` provides Home Assistant (https://www.home-assistant.io) auto-discovery, which means that Home Assistant will automatically detect and configure your MTEC Inverter.
...
`aiomtec_util` is an small inteative tool which enable to list the supported parameters and read and write registers of your Inverter.
...
The command-line tool `aiomtec_export` offers functionality to read data from your Inverter using Modbus and export it in various combinations and formats.
